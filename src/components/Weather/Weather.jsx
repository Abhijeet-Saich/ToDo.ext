...
const Weather = () => {
  ...
  const [location, setLocation] = useState({latitude: "",longitude: "",locality: "",temperature: "",skyStatus: ""});
  ...
  useEffect(() => {
    (async () => {
      try {
        const {data: {main: { temp },name,weather}} = await axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${location.latitude}&lon=${Number(location.longitude)}&appid=${"de134159c8817db45dddd8d164dbaf2c"}&units=metric`);
